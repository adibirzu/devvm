...
def recent_notifications(
    lines: List[str], now_epoch: float, window_sec: int = NOTIFY_WINDOW_SEC
) -> List[Dict[str, Any]]:
...
def recent_guardrail(
    lines: List[str], now_epoch: float, window_sec: int = GUARDRAIL_WINDOW_SEC
) -> List[Dict[str, Any]]:
    ...
    for ln in lines:
        ...
        try:
            ...
            continue
        if (
            ev.get("action") in ("deny", "ask")
            and now_epoch - _parse_iso(ev.get("ts", "")) <= window_sec
        ):
            out.append(ev)
    return out
...
def apply_notifications(
    developers: List[Dict[str, Any]], notifs_by_user: Dict[str, List[Dict[str, Any]]]
) -> int:
    ...
    for dev in developers:
        ...
        dev["needs_input"] = any(
            s.get("needs_input") for s in dev.get("sessions", [])
        ) or bool(evs)
        if dev["needs_input"]:
            ringing_total += 1
...
def fetch_gateway_health(
    gateway: Optional[str], timeout: float = 4.0
) -> Dict[str, Any]:
...
def _read_feed(
    home_root: Path, developers: List[str], now_epoch: float, filename: str, parser
    ...
    for user in developers:
        ...
        if feed.exists():
            out[user] = parser(
                feed.read_text(encoding="utf-8", errors="replace").splitlines(),
                now_epoch,
            )
        else:
            out[user] = []
...
def build(
    developers: List[str], home_root: Path, gateway: Optional[str]
) -> Dict[str, Any]:
    per_user, live = {}, {}
    for user in developers:
    ...
    guardrail = _read_feed(
        home_root, developers, now_epoch, "guardrail.jsonl", recent_guardrail
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Aggregate live agent sessions into a board JSON."
    )
    p.add_argument(
        "--developers",
        default="",
        help="Comma-separated usernames (default: scan --home-root)",
    )
    p.add_argument("--home-root", default="/home")
    p.add_argument(
        "--gateway",
        default="http://10.200.200.1:8080",
        help="Gateway for cost (empty to skip)",
    )
